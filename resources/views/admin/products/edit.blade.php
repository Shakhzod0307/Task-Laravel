@extends('layout')
@section('content')
    <form action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" class="bg-white shadow p-4 py-8" x-data="{ images: [] }">
        @csrf
        @method('PUT')
        <div class="heading text-center font-bold text-2xl m-5 text-gray-800 bg-white">Update Product</div>
        @if(session('success'))
        ...
        <div class="editor mx-auto w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
            <textarea class="description bg-gray-100 sec p-3 h-20 mb-4 border border-gray-300 outline-none" spellcheck="false" name="name" required>{{$product->name}}</textarea>
            ...
            <textarea class="description bg-gray-100 sec p-3 h-40 mb-4 border border-gray-300 outline-none" spellcheck="false" name="short_description" required>{{$product->short_description}}</textarea>
            <textarea class="description bg-gray-100 sec p-3 h-60 border border-gray-300 outline-none" spellcheck="false" name="full_description" required>{{$product->full_description}}</textarea>
            <div class="mb-4">
                <label class="block mb-2 font-semibold">Current Image:</label>
                <img src="{{Storage::url($product->picture)}}" alt="Current Image" class="w-32 h-32 object-cover rounded mb-4">
            </div>
